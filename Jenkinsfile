node {
                    docker.image('node:boron').inside {
                            ...
                            stage ('deploy'){
                                ...
                                sh 'docker service create --name microservice-nodejs --log-driver=gelf --log-opt gelf-address=udp://${env.MANAGER_IP}:12201 --publish 9992:4000 --publish 9993:5000 --network microservices-net --network ${env.APP_NAME}-net --replicas 2 mitosis/microservice-nodejs:1'
                            }
                     }
                 }            \    
